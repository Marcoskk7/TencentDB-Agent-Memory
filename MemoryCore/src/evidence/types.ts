...
export interface TaskRun {
  ...
  variant?: RunVariant; evaluation_group_id?: string; code_version?: EvaluationCodeVersion; asset_mode?: EvaluationAssetMode; asset_selection_mode?: EvaluationAssetSelectionMode; parent_run_id?: string; run_kind?: RunKind;
  model_fingerprint?: string; environment_fingerprint?: string; workspace_id?: string;
...
export interface AssetAccess {
  ...
  selection_score?: number; selection_reason?: string;
  /** Estimated tokens for this asset access; never whole-agent context usage. */
  token_estimate?: number;
  compatibility_risk?: string; applicability?: string; name?: string; source_candidate_id?: string; idempotency_key?: string; created_at: string;
}
...
export interface CodeDiff { diff_id: string; run_id: string; base_commit?: string; head_commit?: string; files: string[]; diff_digest?: string; additions?: number; deletions?: number; idempotency_key?: string; created_at: string; }
...
export interface Review { review_id: string; run_id: string; access_id: string; decision: ReviewDecision; reason: string; behavior_refs?: string[]; diff_refs?: string[]; decision_refs?: string[]; reviewer_user_id: string; idempotency_key?: string; created_at: string; }
export interface Evaluation {
  evaluation_id: string; run_id: string; idempotency_key?: string; access_id?: string;
  comparison_verified?: boolean; control_run_id?: string; baseline_commit?: string;
  environment_fingerprint?: string; passed?: boolean; gain?: number; contamination?: boolean;
  independent_causal_evidence?: boolean; metrics?: Record<string, number>;
  /** Optional benchmark measures retained with the immutable comparison record. */
  task_success?: boolean; tests_passed?: number; repair_time_ms?: number;
  total_tokens?: number; tool_calls?: number; failed_attempts?: number;
  receipt_complete?: boolean; asset_usage_count?: number;
  created_at: string;
}
export interface CandidateAsset { candidate_id: string; source_run_id: string; source_diff_ids: string[]; source_validation_ids: string[]; proposed_kind: "project_convention"|"failure_experience"|"skill"|"workflow"|"code_knowledge"|"product_knowledge"; applicability?: string; risks?: string[]; recommendation?: "publish" | "revise" | "deprecate" | "conflict"; supersedes_asset_id?: string; conflict_asset_ids?: string[]; deprecates_asset_ids?: string[]; version?: number; reviewer_decision?: string; published_asset_id?: string; content: string; confidence: number; review_required: true; status: "candidate" | "rejected" | "approved"; created_at: string; }
...
export interface DerivedAssetEvidence {
  access_id: string; asset_id: string; asset_type: AssetType; name?: string; version: number;
  source_ref?: string; source_candidate_id?: string; applicability?: string; selection_score?: number;
  selection_reason?: string; token_estimate?: number; evidence: EvidenceState;
  decision_refs: string[]; file_refs: string[]; validation_refs: string[]; risks: string[];
